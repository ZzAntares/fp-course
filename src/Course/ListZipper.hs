...
findLeft p (ListZipper l x r) =
  case break p l of
    (a, x' :. xs) -> IsZ $ ListZipper xs x' (reverse a ++ x :. r)
    (_, Nil)      -> IsNotZ
...
findRight p (ListZipper l x r) =
  case break p r of
    (a, x' :. xs) -> IsZ $ ListZipper (reverse a ++ x :. l) x' xs
    (_, Nil)      -> IsNotZ
...
moveLeftLoop (ListZipper (x :. xs) e r) = ListZipper xs x (e :. r)
moveLeftLoop z@(ListZipper Nil e r) =
  let l = reverse (e :. r)
  in case l of
...
moveRightLoop (ListZipper l e (x :. xs)) = ListZipper (e :. l) x xs
moveRightLoop z@(ListZipper l e Nil) =
  let r = reverse (e :. l)
  in case r of
...
moveLeft (ListZipper (x :. xs) e r) = IsZ $ ListZipper xs x (e :. r)
...
moveRight (ListZipper l e (x :. xs)) = IsZ $ ListZipper (e :. l) x xs
...
swapLeft (ListZipper (x :. xs) e r) = IsZ $ ListZipper (e :. xs) x r
...
swapRight (ListZipper l e (x :. xs)) = IsZ $ ListZipper l x (e :. xs)
...
moveLeftN n z
  | n > 1 = case moveLeft z of
      IsZ z' -> moveLeftN (n - 1) z'
      _      -> IsNotZ
  | n < 1 = case moveRight z of
      IsZ z' -> moveLeftN (n + 1) z'
      _      -> IsNotZ
  | otherwise = moveLeft z

-- | Move the focus right the given number of positions. If the value is
-- | negative, move left instead.
--
...
moveRightN n z
  ...
  | n < 1 = case moveLeft z of
      IsZ z' -> moveRightN (n + 1) z'
      _      -> IsNotZ
  | otherwise = moveRight z

-- | Move the focus left the given number of positions. If the value is
-- | negative, move right instead.  If the focus cannot be moved, the given
-- | number of times, return the value by which it can be moved instead.
--
...
-- | Move the focus to the given absolute position in the zipper. Traverse the
-- | zipper only to the extent required.
--
...
nth n z@(ListZipper l _ r)
  | n > zlength = IsNotZ
  | n > length l = moveRightN (n - length l) z
  | otherwise = moveLeftN (length l - n) z
  where zlength = length l + 1 + length r
...
-- prop> \lz -> rights (end lz) == Nil
  ...
  | hasRight z = end $ moveRightLoop z
  | otherwise  = z
...
start z
  | hasLeft z = start $ moveLeftLoop z
  | otherwise = z
...
insertPushLeft e (ListZipper l x r) = ListZipper (x :. l) e r
...
insertPushRight e (ListZipper l x r) = ListZipper l e (x :. r)
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper lf f rf) (ListZipper l x r) = ListZipper ls (f x) rs
    where
      ls = zipWith ($) lf l
      rs = zipWith ($) rf r
...
instance Applicative MaybeListZipper where
  pure = IsZ . pure
  (<*>) (IsZ zf) (IsZ zv) = IsZ $ zf <*> zv
  (<*>) _ _ = IsNotZ


-- | Implement the `Extend` instance for `ListZipper`.
-- This implementation "visits" every possible zipper value derivable from a
-- given zipper (i.e. all zippers to the left and right).
--
