...
-- Note the usage of "seq" function wich forces evaluation of the first
-- parameter, so haskell doesn't behave lazy fill the stack.
foldLeft :: (b -> a -> b) -> b -> List a -> b
foldLeft _ b Nil      = b
foldLeft f b (h :. t) = let b' = f b h in b' `seq` foldLeft f b' t
-- foldLeft is a for loop
-- foldLeft f a xs
-- var accum = a;
-- xs.forEach(x => {
--   accum = f(accum, x);
...
headOr :: a -> List a -> a
-- headOr a xs = foldRight (\e _ -> e) a xs
-- headOr a xs = foldRight P.const a xs
headOr = foldRight P.const
...
-- map f = foldRight (\e -> (f e :.)) Nil
map f = foldRight (\e a -> f e :. a) Nil
...
filter f = foldRight (\e a -> if f e then e :. a else a) Nil
...
-- flatten = foldRight (\es acc -> es ++ acc) Nil
...
-- seqOptional = foldRight sequence (Full Nil)
--   where
--     sequence :: Optional a -> Optional (List a) -> Optional (List a)
--     sequence (Full x) (Full xs) = Full $ x :. xs
--     sequence _ _                = Empty
...
find p = foldRight (\e acc -> if p e then Full e else acc) Empty
...
-- reverse = foldRight (\e acc -> acc ++ (e :. Nil)) Nil
